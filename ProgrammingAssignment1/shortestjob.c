...
void runShortestJobFirst(FILE *ofp, process *processes, int numProcesses, int runfor)
{
    ...
    while (tick < runfor)
    {
        ...
        for (int i = 0; i < numProcesses; i++)
        {
            ...
            {
                printStatusLine(ofp, tick, &processes[i], shortestBurst->burst, "arrived");
            }
        ...
        printStatusLine(ofp, tick, shortestBurst, shortestBurst->burst, "selected");
        ...
        {
            printStatusLine(ofp, ++tick, shortestBurst, 0, "finished");
        }
